...
def test_path_request_openapi_example(client):
    input_data = read_json_file(TEST_REQ_DIR / "planning_demand_example.json")

    response = client.get("/openapi.json")
    body = response.json()["paths"][f"{API_VERSION}/path-request"]["post"]["requestBody"]["content"]

    assert body["application/json"]["examples"]["planning-demand"]["value"] == input_data
    assert body["application/json"]["schema"]["type"] == "object"
    assert body["application/json"]["schema"]["example"] == input_data
